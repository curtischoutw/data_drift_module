...
class ImprovedDriftVisualizer:
    ...
    def __init__(self, detector, current_data: pd.DataFrame):
        ...
        self.current_data = current_data  # 修正: 儲存當前資料
    ...
    def plot_numerical_drift(self, feature_name: str, figsize=(16, 10), save_path=None):
        ...
        curr_data = self.current_data[feature_name].dropna()  # 修正:使用儲存的 current_data
        ...
        # 正規化數值以便比較
        max_val = max(max(ref_values), max(curr_values))
        min_val = min(min(ref_values), min(curr_values))
        ...
        for test_name, test_result in result['tests'].items():
            if test_name == 'ks_test':
                ...
                tests_info.append(f"✓ Wasserstein Distance: {test_result['value']:.4f} ({test_result['interpretation']})")
        
        # 加入缺失值資訊
        missing = result.get('missing_analysis', {})
        if missing:
            tests_info.append("")
            tests_info.append("【缺失值分析】")
            tests_info.append(f"參考資料缺失率: {missing['reference_missing_rate']:.2%}")
            tests_info.append(f"當前資料缺失率: {missing['current_missing_rate']:.2%}")
            if missing['has_drift']:
                tests_info.append(f"⚠️ 缺失率有顯著變化!")
    ...
    def plot_categorical_drift(self, feature_name: str, figsize=(14, 10), save_path=None):
        ...
        ref_data = self.reference_data[feature_name].dropna()
        curr_data = self.current_data[feature_name].dropna()  # 修正
        ...
        ax2.barh(all_categories, diff, color=colors, alpha=0.7)
        ...
        ax3.pie(ref_props, labels=all_categories, autopct='%1.1f%%',
               startangle=90)
        ax3.set_title('參考資料分布', fontweight='bold', fontsize=12)
        
        # 4. 圓餅圖比較 (當前資料)
        ax4 = fig.add_subplot(gs[2, 0])
        ax4.pie(curr_props, labels=all_categories, autopct='%1.1f%%',
               startangle=90)
        ...
        for test_name, test_result in result['tests'].items():
            if test_name == 'chi_square_test':
                ...
                tests_info.append(f"✓ TVD: {test_result['value']:.4f}")
                tests_info.append(f"  ({test_result['interpretation']})")
            elif test_name == 'hellinger_distance':
                tests_info.append(f"✓ Hellinger: {test_result['value']:.4f}")
                tests_info.append(f"  ({test_result['interpretation']})")
    ...
    def plot_metrics_comparison(self, figsize=(14, 8), save_path=None):
        ...
        for feature, result in self.drift_results['feature_results'].items():
            ...
            numerical_features.append(result['feature_type'] == 'numerical')
        ...
        if drift_reasons_all:
            ...
            axes[0, 1].text(0.5, 0.5, '無漂移檢測', 
                           ha='center', va='center', fontsize=14)
            axes[0, 1].axis('off')
        
        # 3. 數值型 vs 類別型漂移比例
        numerical_drifted = 0
        categorical_drifted = 0
        numerical_total = 0
        categorical_total = 0
        ...
        categories = ['數值型', '類別型']
        drifted = [numerical_drifted, categorical_drifted]
        total = [numerical_total, categorical_total]
        no_drift = [total[i] - drifted[i] for i in range(len(total))]
        
        x = np.arange(len(categories))
        width = 0.35
        
        axes[1, 0].bar(x, drifted, width, label='有漂移', color='#ff6b6b', alpha=0.7)
        axes[1, 0].bar(x, no_drift, width, bottom=drifted, label='無漂移', 
        ...
        if features_with_missing:
            ...
            axes[1, 1].text(0.5, 0.5, '無顯著缺失值變化', 
                           ha='center', va='center', fontsize=14)
            axes[1, 1].axis('off')
    ...
    def plot_all_drifts(self, save_dir=None):
        ...
        # 繪製指標比較
        metrics_path = f"{save_dir}/metrics_comparison.png" if save_dir else None
...
# 快速創建函數
def quick_drift_visualization(ref_data: pd.DataFrame, 
    ...
    feature_name : str
        要比較的特徵名稱
    """
    if feature_name not in ref_data.columns or feature_name not in curr_data.columns:
        print(f"特徵 '{feature_name}' 不存在於資料中")
        return
    ...
    if pd.api.types.is_numeric_dtype(ref_values):
        # 數值型:直方圖
        axes[0].hist(ref_values, bins=30, alpha=0.6, label='參考資料', 
                    color='blue', density=True, edgecolor='black')
        axes[0].hist(curr_values, bins=30, alpha=0.6, label='當前資料', 
                    color='red', density=True, edgecolor='black')
        axes[0].set_xlabel('值')
        ...
        ref_props = [ref_counts.get(cat, 0) / len(ref_values) for cat in all_cats]
        curr_props = [curr_counts.get(cat, 0) / len(curr_values) for cat in all_cats]
        
        axes[0].bar(x - width/2, ref_props, width, label='參考資料', alpha=0.7)
        axes[0].bar(x + width/2, curr_props, width, label='當前資料', alpha=0.7)
        axes[0].set_xticks(x)
        ...
        # 差異圖
        diff = [curr_props[i] - ref_props[i] for i in range(len(all_cats))]
        colors = ['red' if d > 0 else 'blue' for d in diff]
        axes[1].barh(all_cats, diff, color=colors, alpha=0.7)
        axes[1].set_xlabel('比例差異 (當前 - 參考)')
        axes[1].set_title(f'{feature_name} - 變化方向')
